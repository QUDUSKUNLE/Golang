...
import (
	...
	address "github.com/QUDUSKUNLE/microservices/address-service/core/domain"
	// models "github.com/QUDUSKUNLE/microservices/auth-service/internal/models"
	// carrier "github.com/QUDUSKUNLE/microservices/carrier-service/core/domain"
	// pick_up "github.com/QUDUSKUNLE/microservices/pickup-service/core/domain"
...
type (
	Shipping struct {
		...
		// Carrier            *carrier.Carrier `json:"-"`
		ProductType        ProductType      `json:"Product_type"`
		// PickUp             pick_up.PickUp   `json:"-"`
	...
	SingleShippingDto struct {
		...
		PickUpAddress      address.Address `json:"pickup_address" validate:"required_without=PickUpAddressID"`
		DeliveryAddress    address.Address `json:"delivery_address" validate:"required_without=DeliveryAddressID"`
		TerminalShipmentID string          `json:"terminal_shipment_id"`
		ProductType        ProductType     `json:"product_type" validate:"required"`
		CarrierID          uuid.UUID       `json:"carrier_id" validate:"uuid,required"`
		UserID             uuid.UUID
	}
	ShipmentParamDto struct {
		ShipmentID uuid.UUID `param:"shipment_id" validate:"required,uuid"`
	}
)
